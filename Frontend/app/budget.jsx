...
import {StyleSheet,View,Text,TextInput,TouchableOpacity,Platform,StatusBar,ScrollView,Alert,ActivityIndicator,} from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { useFocusEffect } from '@react-navigation/native';
import { router } from 'expo-router';
import Header from '../component/header';
import { getBudgets, saveBudget } from '../utils/api';
...
const BudgetSettingsScreen = () => {
      ...
      const loadBudget = async () => {
        try {
          ...
          if (budgets && budgets.length > 0) {
            ...
            setNeedsPercentage(latestBudget.needs_percent.toString());
            setWantsPercentage(latestBudget.wants_percent.toString());
            setSavingsPercentage(latestBudget.savings_percent.toString());
            
            // If already saved, don't show as custom
            if (
              latestBudget.needs_percent === 50 &&
              latestBudget.wants_percent === 30 &&
              latestBudget.savings_percent === 20
            ) {
              setSelectedRule('503020');
            } else {
  ...
  const handleSave = async () => {
    if (exceedsLimit) {
      Alert.alert('Error', 'Budget allocation cannot exceed 100%');
      return;
    }
    ...
    try {
      const budgetData = {
        needs_percent: parseInt(needsPercentage),
        wants_percent: parseInt(wantsPercentage),
        savings_percent: parseInt(savingsPercentage),
      };
